...
summarize.match <- function(dat, ms, ps.name="prop") {
    ...
    adat.treat <- adat[adat$ret_pro==1, ]
    adat.ctrl <- adat[adat$ret_pro==0, ]

    adat.m <- merge(adat.treat, adat.ctrl, by="pair", suffixes=c(".1", ".0"))
    adat.m <- adat.m[, -which(names(adat.m) %in% c("ret_pro.1", "ret_pro.0", "pair"))]
...
# compute t-test between ret_pro==1 and ret_pro==0
ret_pro_t_test <- function(input_df, input_field){
  t_test_obj <- t.test(input_df[input_df$ret_pro==1, input_field], input_df[input_df$ret_pro==0, input_field])
...
sped_var <- c("ret_pro") # indicator of whether received special education 
...
fit <- glm(ret_pro ~ . -ctr, family = binomial(), data = df_no_na)
...
```{r}
print("Math")
# with(df_no_na, t.test(ctr ~ ret_pro))
ret_pro_t_test(df_no_na, "ctr")

```

### baseline ols (no covariates) - SHOULD MATCH T-TEST
```{r}
no_covar_ols_math <- lm(ctr ~ ret_pro, data = df_no_na)
print("Math")
...
round(summary(covar_ols_math)$coefficients[c("ret_pro"), ], 2)
```

## method 1: nearest neighbor matching (from paper)
```{r}
fmla <- as.formula(paste("ret_pro ~ ", paste(handcraft_var, collapse= "+")))

ms.1 =  pairmatch(match_on(glm(fmla,data = df_no_na,family = binomial())), data=df_no_na)

#plot(xBalance(ret_pro ~ . -prop -ctr -c5r2rscl + strata(ms.1) - 1, data =  df_no_na),main="Covariate balance with nearest neighbors")
df_imp <- subset(df_no_na, select=important_var)
plot(xBalance(ret_pro ~ . + strata(ms.1) - 1, data =  df_imp),main="Covariate balance with nearest neighbors")
...
print("Math")
ret_pro_t_test(df_nn_match, "ctr")

# viz
# here, I plot for the variables the paper claims as important
ggplot(data=df_nn_match, aes(x=prop, group=as.factor(ret_pro), fill=as.factor(ret_pro))) + geom_density(alpha=1) + theme_bw()
...
```{r}
mat.2 <- smahal(df_no_na$ret_pro, subset(df_no_na, select=c(-ret_pro, -ctr, -prop)))
ms.2 <- pairmatch(mat.2, data = df_no_na)
#plot(xBalance(ret_pro ~ . -prop -ctr -c5r2rscl + strata(ms.2) - 1, data=df_no_na),main="Balance with Mahalanobis matching")

df_imp <- subset(df_no_na, select=important_var)
plot(xBalance(ret_pro ~ . + strata(ms.2) - 1, data =  df_imp),main="Balance with Mahalanobis matching")
...
print("Math")
ret_pro_t_test(df_mahalobnis, "ctr")
...
```{r}
mat.3 <- addcaliper(mat.2, z=df_no_na$ret_pro, p=df_no_na$prop, caliper=0.1)
ms.3 <- pairmatch(mat.3, data=df_no_na)
#plot(xBalance(ret_pro ~ . -prop -ctr -c5r2rscl + strata(ms.3) - 1, data=df_no_na),main="Balance with Mahalanobis matching after caliper")

df_imp <- subset(df_no_na, select=important_var)
plot(xBalance(ret_pro ~ . + strata(ms.3) - 1, data =  df_imp),main="Balance with Mahalanobis matching after caliper")
...
```{r}
mat.4 <- addalmostexact(mat.3, z=df_no_na$ret_pro, f=df_no_na$media_type, mult=5)
ms.4 <- pairmatch(mat.4, data=df_no_na)
#plot(xBalance(ret_pro ~ . -prop -ctr -c5r2rscl + strata(ms.4) - 1, data=df_no_na),main="Balance after exact matching on disability status")

df_imp <- subset(df_no_na, select=important_var)
plot(xBalance(ret_pro ~ . + strata(ms.4) - 1, data =  df_imp),main="Balance after exact matching on disability status")

summ.4 <- summarize.match(df_no_na,ms.4)
abs_diff.4 = abs(summ.4$prop.1 - summ.4$prop.0)
cat("Mean absolute diff in prop scores after exact matching on media_type status = ",mean(abs_diff.4), "\n" )
cat("Max absolute diff in prop scores after exact matching on media_type status = ",max(abs_diff.4))

df_almostexact <- cbind(df_no_na, ms.4)
df_almostexact <- na.omit(df_almostexact)

print("Math")
ret_pro_t_test(df_almostexact, "ctr")
...
### Math
```{r}
preproc_nn_match_math <- cast.senm(dat=df_no_na, ms.arg=ms.1, y_field="ctr", z_field="ret_pro")
...
### Math
```{r}
preproc_caliper_match_math <- cast.senm(dat=df_no_na, ms.arg=ms.3, y_field="ctr", z_field="ret_pro")
...
### Math
```{r}
preproc_exact_match_math <- cast.senm(dat=df_no_na, ms.arg=ms.4, y_field="ctr", z_field="ret_pro")
